...
export function AuthProvider({children}:{children:ReactNode}) {
 ...
 async function load(next:Session|null){setLoading(true);setSession(next);setProfile(null);setUnit(null);if(!next){setLoading(false);return;}const {data,error}=await supabase.from("roys_profiles").select("user_id,display_name,username,role,unit_id,active,roys_units(id,name,active)").eq("user_id",next.user.id).single();if(error||!data||!data.active){await supabase.auth.signOut();setSession(null);setLoading(false);return;}setProfile({userId:data.user_id,displayName:data.display_name,username:data.username??undefined,role:data.role,unitId:data.unit_id??undefined,active:data.active});const related=Array.isArray(data.roys_units)?data.roys_units[0]:data.roys_units;if(related)setUnit({id:related.id,name:related.name,active:related.active});setLoading(false);}
