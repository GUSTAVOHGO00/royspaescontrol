...
function ensure<T>(data:T,error:{message:string;code?:string}|null):T{if(error)throw new Error(error.message);return data}
function isDuplicateClosing(error:{message:string;code?:string}|null){return Boolean(error&&(error.code==="23505"||error.message.includes("roys_closings_one_original_per_shift")))}
export function createCloudClosingRepository(client:SupabaseClient){return{
 async submitClosing(input:CloudClosingInput):Promise<ClosingReceipt>{const {data,error}=await client.rpc("submit_store_closing",{payload:input});if(isDuplicateClosing(error))throw new DuplicateClosingError("");return ensure(data,error) as ClosingReceipt;},
